...
#	configurations.	Finally, the script deletes ports/cards one by one.	 	   #	
#                                                                              #
################################################################################
...
if {[keylget clear_chassis status] != $::SUCCESS} {
   ...
   return 0
	...
	puts "Retrieving current virtual chassis topology ..\n"

	set info [ixiangpf::ixvm_info				\
		-virtual_chassis		$chassis_ip		\
	]
	if {[keylget info status] != $::SUCCESS} {
	   puts "Info failed: $info"
	   return 0
	}
	
	set info_cards [keylkeys info virtual_cards]
	foreach c $info_cards {
		set aux [keylkeys info virtual_cards.${c}.virtual_ports]
		foreach x $aux {
			puts "Deleting port ${c}.${x}!"
			set del [ixiangpf::ixvm_config					\
				-virtual_chassis			$chassis_ip		\
				-mode						modify			\
				-card_no					$c				\
				-port_action				delete			\
			...
			if {[keylget del status] != $::SUCCESS} {
				puts "Port delete failed: $del"
				return 0
			}

		}
		puts "Finished with card no $c\n"
	}
