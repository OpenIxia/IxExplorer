...
#    The client and server networks are configured with IPSec                  #
...
set client_network_range [::ixia::L47_network             \
        ...
        -mode                add                        \
        -np_first_ip         "150.0.0.1"                \
        -np_network_mask     "255.255.0.0"              \
        -np_ip_incr_step     "0.0.0.1"                  \
        -np_ip_count         100                        \
        -np_range_type                  ipsec           \
        -np_role                        p2p_initiator   \
        -np_emulated_subnet             "140.0.0.0"     \
        -np_emulated_subnet_mask        "255.255.255.0" \
        -np_protected_subnet            "170.0.0.0"     \
        -np_protected_subnet_mask       "255.255.255.0" \
        -np_increment_by                "0.0.1.0"       \
        -np_peer_public_ip              "160.0.0.1"     \
        -np_emulated_hosts              1               \
        -np_pre_shared_key              "ixload"        \
        -np_ike_mode                    main            \
        -np_phase1_hash_algorithm       md5             \
        -np_dh_group                    dh2             \
        -np_phase1_encryption_algorithm 3des            \
        -np_phase1_lifetime             3600            \
        -np_ah_esp_options              esp             \
        -np_phase2_hash_algorithm       md5             \
        -np_phase2_encryption_algorithm 3des            \
        -np_phase2_lifetime             28800           \
        -np_enable_pfs                  0               \
        -np_num_retries                 0               \
        -np_retry_interval              10              \
        -np_retry_delay                 10              ]
...
set client_network [::ixia::L47_network             \
        -handle            $client_network_handle \
        -target            client                  \
        -property          router_pool            \
        -mode              add                      \
        -rp_first_ip       "155.0.0.1"            \
        -rp_ip_count       1                      \
        -rp_first_mac      "00:9B:00:00:01:00"    ]
        
if {[keylget client_network status] != $::SUCCESS} {
    return "FAIL - $test_name - [keylget client_network log]"
}
set client_router_handle [keylget client_network router_pool_handle]
...
set server_network_range [::ixia::L47_network             \
        ...
        -property           network_pool                \
        -mode               add                         \
        -np_first_ip        "160.0.0.1"                 \
        -np_network_mask    "255.255.0.0"               \
        -np_ip_incr_step    "0.0.0.1"                   \
        -np_ip_count        100                         \
        -np_range_type                  ipsec           \
        -np_role                        p2p_responder   \
        -np_emulated_subnet             "170.0.0.0"     \
        -np_emulated_subnet_mask        "255.255.255.0" \
        -np_protected_subnet            "140.0.0.0"     \
        -np_protected_subnet_mask       "255.255.255.0" \
        -np_increment_by                "0.0.1.0"       \
        -np_peer_public_ip              "150.0.0.1"     \
        -np_emulated_hosts              1               \
        -np_pre_shared_key              "ixload"        \
        -np_ike_mode                    main            \
        -np_phase1_hash_algorithm       md5             \
...
set client_network [::ixia::L47_network             \
        ...
        -property          router_pool            \
        -mode              add                      \
        -rp_first_ip       "155.0.0.2"            \
        -rp_ip_count       1                      \
        -rp_first_mac      "00:9B:00:00:02:00"    ]
        
if {[keylget client_network status] != $::SUCCESS} {
    return "FAIL - $test_name - [keylget client_network log]"
}
set server_router_handle [keylget client_network router_pool_handle]
...
set server_stat_handle [keylget ftp_server_stat handles]
