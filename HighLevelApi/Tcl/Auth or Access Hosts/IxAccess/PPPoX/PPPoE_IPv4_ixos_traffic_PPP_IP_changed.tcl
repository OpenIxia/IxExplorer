...
#                                                                              #
# Description:                                                                 #
#    This sample configures a PPPoE tunnel with sessions between the           #
...
# 
# ip local pool pool21IPv4 21.21.21.2 21.21.21.254
# 
# interface FastEthernet5/0
#  no ip address
#  no ip route-cache cef
#  no ip route-cache
#  pppoe enable group group21IPv4
#  no shutdown
# 
...
#  peer default ip address pool pool21IPv4
#  no keepalive
#  ppp max-bad-auth 20
...
while {($pppoe_attempts < 20) && ($pppoe_sessions_up < $sess_count)} {
    ...
    if {[keylget pppox_status status] != $::SUCCESS} {
        puts "FAIL - $test_name - [keylget pppox_status log]"
        return
    }
...
set traffic_status [::ixia::traffic_config                \
        ...
        -ip_src_addr                    22.22.22.2        \
        -ip_src_step                    0.0.0.1           \
        -ip_dst_mode                    fixed             \
        -ip_dst_addr                    101.101.101.110   \
...
if {[keylget session_status status] != $::SUCCESS} {
   puts "FAIL - $test_name - [keylget session_status log]"
   return
}
