...
proc setup_dhcp_relay_agent {topology_1_handle} {
    ############################################
    ## dhcpv4 relay agent device group
    ############################################
    ...
    set mv_ipv4_resolve_gateway_status [::ixiangpf::multivalue_config \
        -pattern                counter                 \
        -counter_start          0                       \
        -counter_step           0                       \
        -counter_direction      increment               \
    ...
    if {[keylget mv_ipv4_resolve_gateway_status status] != $::SUCCESS} {
        errorHandler [info script] $mv_ipv4_resolve_gateway_status
    }
    set mv_ipv4_resolve_gateway_handle [keylget mv_ipv4_resolve_gateway_status multivalue_handle]

    set ipv4_status [::ixiangpf::interface_config \
        -protocol_name                {IPv4 1}                          \
        -protocol_handle              $ethernet_handle                  \
        -ipv4_resolve_gateway         $mv_ipv4_resolve_gateway_handle   \
        -ipv4_manual_gateway_mac      00.00.00.00.00.01                 \
        -gateway                      0.0.0.0                           \
        -intf_ip_addr                 $mv_intf_ip_addr_handle           \
    ...
    set dhcpv4relay_agent_status [::ixiangpf::emulation_dhcp_group_config \
        -dhcp_range_ip_type                  ipv4                                       \
        -mode                                create_relay_agent                         \
        -handle                              $ipv4_handle                               \
        -protocol_name                       {DHCPv4 Relay Agent 1}                     \
    ...
    if {[keylget dhcpv4relay_agent_status status] != $::SUCCESS} {
        errorHandler [info script] $dhcpv4relay_agent_status
    }
    set dhcpv4relayAgent_handle [keylget dhcpv4relay_agent_status dhcpv4relayagent_handle]
...
proc setup_dhcp_client_linked {topology_1_handle deviceGroup_1_handle ethernet_1_handle} {
    ...
    set dhcpv4client_status [::ixiangpf::emulation_dhcp_group_config \
        -dhcp_range_ip_type               ipv4                    \
    ...
    if {[keylget dhcpv4client_status status] != $::SUCCESS} {
        errorHandler [info script] $dhcpv4client_status
    }
    set dhcpv4client_handle [keylget dhcpv4client_status dhcpv4client_handle]
...
set ipv4_1_handle               [lindex $ret 2]
set dhcpv4relayAgent_1_handle   [lindex $ret 3]
...
set dhcpv4client_1_handle       [lindex $ret 2]
