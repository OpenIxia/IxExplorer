...
for {set i 2} {$i <= [expr $tunnel_count + 1]} {incr i} {
    ...
    lappend speed_list               ether1000
    lappend phy_mode_list            copper
    lappend auto_list                1
    lappend netmask_list             255.255.255.0
    lappend src_mac_addr_list        00cd.00cd.[format "%04x" $i]
    lappend qos_stats_list           1
    lappend qos_byte_offset_list     55
    lappend qos_pattern_offset_list  55
    lappend qos_pattern_match_list   00
    lappend qos_pattern_mask_list    FF
    lappend qos_packet_type_list     custom
}
...
set interface_status [::ixia::interface_config       \
        ...
        -qos_stats          $qos_stats_list          \
        -qos_byte_offset    $qos_byte_offset_list    \
        -qos_pattern_offset $qos_pattern_offset_list \
        -qos_pattern_match  $qos_pattern_match_list  \
        -qos_pattern_mask   $qos_pattern_mask_list   \
        -qos_packet_type    $qos_packet_type_list    ]
